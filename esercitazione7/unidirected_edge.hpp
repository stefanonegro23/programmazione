...
class unidirected_edge{
    ...
    public:
        ...
        bool operator<(const unidirected_edge& other) const { //la & mi permette di referenziare un elemento (estraendone il valore) senza fare una copia
...
class unidirected_graph {
    ...
    nodi.insert(nodo);
        ...
        unidirected_edge<I> edge_at(int posizione){
            int i = 0;
            for (const auto& arco:archi){ //uso l'iteratore 
                if (i==posizione){
                    return arco;
                }
                else{
        ...
        double restituisci_peso(I nodo1, I nodo2) const{
            unidirected_edge<I> arco_cercato(nodo1,nodo2);
            auto it = archi.find(arco_cercato);
            if (it != archi.end()) {
                return it ->get_peso(); // Estraiamo il peso
            }
            return std::numeric_limits<double>::infinity(); //se l'arco non esiste
        }
};